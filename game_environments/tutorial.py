...
from settings import (screen_width, screen_height, thirds, cx, cy, text_color, font_small, warning_radius,
                      enemies_count_rate, player_projectile_radius, player_damage, player_projectile_color,
                      adapt_from_stage)
from entities.enemy import enemies, spawn_enemies, spawn_tutorial_unit, init_agent, update_profile
...
class InputButton:
    ...
    def handle_events(self, event):
        ...
        if self.mouse_button:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == self.mouse_button:
                self.highlighted = True
                self.pressed     = True
    ...
    def draw(self, surface):
        ...
        color = "black" if self.highlighted else "white"
        text  = font_small.render(self.letter, True, color)
...
class Tutorial:
    ...
    def init_tutorial(self):
        ...
        self.tutorial_buttons = {
            "W": InputButton(base_x,       base_y - gap, button_size, "W", pygame.K_w),
            "A": InputButton(base_x - gap, base_y,       button_size, "A", pygame.K_a),
            "S": InputButton(base_x,       base_y,       button_size, "S", pygame.K_s),
            "D": InputButton(base_x + gap, base_y,       button_size, "D", pygame.K_d),
        }
        self.tutorial_mouse_buttons = {
            "1": InputButton(cx - button_size // 2, base_y, button_size, "Mouse 1", mouse_button=1),
        }
    ...
    def handle_tutorial_event(self, event):
        ...
        for button in self.tutorial_mouse_buttons.values():
            button.handle_events(event)
    ...
    def draw_tutorial(self, screen):
        ...
        if self.tutorial_stage == 1:
            msg = font_small.render("Press W A S D to Move", True, text_color)
    ...
    def save_log(self):
        ...
        print(f"[save_log] saving to: {path}")
        print(f"[save_log] frames captured: {len(self.behavior_log)}")
        print(f"[save_log] shots fired={self.shots_fired} hit={self.shots_hit}")
        try:
            os.makedirs(path, exist_ok=True)
        except OSError as e:
            print(f"Failed to create replay directory {path}: {e}")
            raise
    ...
    def _update_evasion(self):
        ...
        for ep in self.enemy_projectiles:
            ...
            if dist < warning_radius and eid not in self._tracked_projectiles:
                self._tracked_projectiles[eid] = {
                    "start_pos":  [ep.pos.x, ep.pos.y],
    ...
    def update(self, dt):
        ...
        if not self.tutorial_done:
            if all(b.pressed for b in self.tutorial_buttons.values()):
                self.tutorial_timer += dt
                if self.tutorial_timer >= 2.0:
