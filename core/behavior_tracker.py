def compute_live_profile(behavior_log: list, stage: int) -> dict:
    ...
    total_moves = max(approaching + retreating, 1)
    approach_ratio = round(approaching / total_moves, 3)
    avg_range = sum(dist_samples) / max(len(dist_samples), 1)
    aggression_score = approach_ratio
