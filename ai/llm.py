...
def synthesis(log_path, profile_name):
    ...
    deltas = [dist_samples[i] - dist_samples[i-1] for i in range(1, len(dist_samples))]
    approaching    = sum(1 for d in deltas if d < -5)
    retreating     = sum(1 for d in deltas if d > 5)
    approach_ratio = round(approaching / max(approaching + retreating, 1), 3)
...
Field guide:
- approach_ratio: 0=always retreating from enemies, 1=always closing in. Use this as the primary aggression indicator.
- aggression_score: base this on approach_ratio, not shots_fired
...
- summary: using all information from the session give a summary of the game and take a guess of the users personality + myers briggs personality test result
