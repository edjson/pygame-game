...
class PauseMenu:
    """Pause overlay with Resume, Settings, and Main Menu buttons."""

    def __init__(self, manager):
        """Create the three pause buttons and hide them until the menu is shown."""
    ...
    def handle_event(self, event):
        """Return 'resume', 'main', or 'settings' on button press, else None."""
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if event.ui_element == self.resume_button:   return "resume"
            if event.ui_element == self.quit_button:     return "main"
